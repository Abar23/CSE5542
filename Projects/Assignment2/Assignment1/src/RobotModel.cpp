...
void RobotModel::RotateRobotGlobally(glm::mat4 * rotationMatrix)
{
	ModelNode *body = this->hierarchy->GetRootNode();

	body->GetShape()->RotateModelMatrixGlobally(rotationMatrix);

	// Update left leg
	UpdateAppendageGlobalRotation(LEFT_LEG_CHILD_INDEX, NUM_LEG_COMPONENTS, body, rotationMatrix);

	// Update right leg
	UpdateAppendageGlobalRotation(RIGHT_LEG_CHILD_INDEX, NUM_LEG_COMPONENTS, body, rotationMatrix);

	// Update head
	UpdateAppendageGlobalRotation(HEAD_CHILD_INDEX, NUM_HEAD_COMPONENTS, body, rotationMatrix);

	// Update left arm
	UpdateAppendageGlobalRotation(LEFT_ARM_CHILD_INDEX, NUM_ARM_COMPONENTS, body, rotationMatrix);

	// Update right arm
	UpdateAppendageGlobalRotation(RIGHT_ARM_CHILD_INDEX, NUM_ARM_COMPONENTS, body, rotationMatrix);
}
...
void RobotModel::UpdateAppendageGlobalRotation(unsigned int childIndexOfModel, unsigned int numberOfComponents, ModelNode * body, glm::mat4 * globalRotation)
{
	// Get ppendage at the passed in position
	ModelNode *node = body->GetChildAt(childIndexOfModel);
	// Iterate through all components of the appendage and apply their corresonding transformations
	for (unsigned int i = 0; i < numberOfComponents; i++)
	{
		node->GetShape()->RotateModelMatrixGlobally(globalRotation);
		if (i != numberOfComponents - 1)
		{
			node = node->GetChildAt(0);
		}
